...
// SubagentTool 存在于 tools 包，完整的 AgentEngine 存在于 engine 包。
...
func (t *SubagentTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	log.Printf("[Subagent] 🚀 主 Agent 发起委派！正在拉起Subagent: [%s]...\n", input.TaskPrompt)

	// 拉起一个完全物理隔离的子循环
	// 把针对该任务的专项指令传给子智能体，并仅提供 readOnlyRegistry。
	// (子智能体只能读文件或执行只读的 bash，不能搞破坏)
	summary, err := t.runner.RunSub(ctx, input.TaskPrompt, t.readOnlyRegistry, t.reporter)

	if err != nil {
		return fmt.Errorf("Subagent执行失败: %w", err).Error(), nil
	}

	log.Printf("[Subagent] ✅ 子智能体任务结束。报告返回给主干...")

	// 像一次普通的 API 调用一样，返回总结给主 Agent。
	return fmt.Sprintf("【Subagent报告】:\n%s", summary), nil
}
